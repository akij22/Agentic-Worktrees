...
const summary = (
  state: CapabilitySummaryDto["state"],
  compatibility: CapabilitySummaryDto["compatibility"] = {
    codex: "supported",
    opencode: "supported",
  },
): CapabilitySummaryDto => ({
...
const detail: CapabilityDetailDto = {
  ...
  license: "MIT",
  permissions: { network: ["mcp.exa.ai"], secrets: [] },
  settings: [],
  reviewStatus: "bundled-reviewed",
  activeRunCount: 0,
...
describe("CapabilityPicker", () => {
  it("groups capabilities and activates ready or failed entries", async () => {
    const incompatible = {
      ...summary("ready", { codex: "unsupported", opencode: "supported" }),
      id: "cap-incompatible",
    };
    const { onActivate } = renderPicker([
      summary("active"),
      summary("ready"),
      summary("needs_setup"),
      summary("activation_failed"),
      incompatible,
    ]);
    await userEvent.click(screen.getByRole("button", { name: "Capabilities" }));
    for (const group of ["Active", "Ready", "Needs setup", "Unavailable"])
      expect(screen.getByText(group, { selector: "p" })).toBeTruthy();
    await userEvent.click(
      screen.getByRole("option", {
        name: /Web Search activation_failed.*Retry/i,
      }),
    );
  ...
  it("configures keyless setup and then activates", async () => {
    ...
    Object.defineProperty(window, "api", {
      configurable: true,
      value: {
        capabilities: { get: vi.fn().mockResolvedValue(detail), configure },
      },
    });
    const { onActivate } = renderPicker([summary("needs_setup")]);
    await userEvent.click(screen.getByRole("button", { name: "Capabilities" }));
    await userEvent.click(
      screen.getByRole("option", { name: /Web Search needs_setup/i }),
    );
    await userEvent.click(
      await screen.findByRole("button", { name: "Accept and continue" }),
    );
    expect(configure).toHaveBeenCalledWith(
      expect.objectContaining({ capabilityId: "cap-needs_setup" }),
    );
  ...
  it("confirms removal and prevents incompatible activation", async () => {
    ...
    const incompatible = summary("ready", {
      codex: "unsupported",
      opencode: "supported",
    });
    const { onActivate, onDeactivate } = renderPicker([
      summary("active"),
      incompatible,
    ]);
    await userEvent.click(screen.getByRole("button", { name: "Capabilities" }));
    expect(
      (
        screen.getByRole("option", {
          name: /Web Search ready/i,
        }) as HTMLButtonElement
      ).disabled,
    ).toBe(true);
    await userEvent.click(
      screen.getByRole("option", { name: /Web Search active/i }),
    );
