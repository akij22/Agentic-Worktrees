...
export class CapabilityDistributionService {
  ...
  private busy = false;
  constructor(private readonly deps: { layout: ManagedPackageLayout; acquirer?: NpmPackageAcquirer; inspector?: CapabilityPackageInspector; verifier: CapabilityPackageVerifier; installer?: CapabilityPackageInstaller; repository?: ManagedPackageRepository; officialCatalog?: OfficialCatalogService }) {
  ...
  async inspect(input: PackageInspectRequest): Promise<CapabilityPackageInspectionDto> {
    ...
    try {
      const staged = await this.acquirer.acquire(id, input.sourceSpec, controller.signal, stage => this.emit(id, stage, "in_progress"));
  ...
  async install(input: PackageInstallRequest): Promise<CapabilityDetailDto> {
    ...
    this.busy = true; try { const verification = await this.deps.verifier.verify(found.inspected, new AbortController().signal); const record = await this.installer.commitFresh(found.inspected, verification); this.inspections.delete(input.inspectionId); this.busy = false; return { ...found.inspected.descriptor as unknown as CapabilityDetailDto, source: "npm", state: record.state === "installed" ? "ready" : "needs_setup" }; } finally { this.busy = false; }
  }
