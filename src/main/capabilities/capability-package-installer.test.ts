...
describe("CapabilityPackageInstaller real fixtures", () => {
  ...
  it("preserves an exact legacy configuration when migration commits the managed package", async () => { const f = await fixture(); f.capabilities.saveConfiguration({ capabilityId: f.capabilityId, version: f.version, permissionDigest: "perm", configured: true }, [{ key: "mode", value: "legacy" }, { key: "apiKey", secretRef: "encrypted-ref" }]); const before = f.capabilities.snapshotInstalledConfiguration(f.capabilityId); await new CapabilityPackageInstaller(f.layout, f.repo, f.capabilities, (work) => f.db.transaction(work)()).commitFresh(f.inspected, { capabilityId: f.capabilityId, version: f.version, contentDigest: f.contentDigest, toolNames: [] }, undefined, undefined, true); expect(f.capabilities.snapshotInstalledConfiguration(f.capabilityId)).toEqual(before); });
