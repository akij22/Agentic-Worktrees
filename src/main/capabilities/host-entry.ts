...
if (!port)
  throw new Error("Capability host requires a utility-process parent port.");
...
port.on("message", async (event: unknown) => {
  ...
  try {
    if (candidate.type === "host.initialize") {
      ...
      await server.setActiveCapabilities(
      ...
      if (!server)
        throw new CapabilityError(
          "internal_error",
          "Capability host is not initialized.",
        );
      const toolNames = await server.setActiveCapabilities(
        candidate.capabilities,
        candidate.settings,
      );
      send({
    ...
    const safe =
      error instanceof CapabilityError
        ...
        : new CapabilityError(
            "internal_error",
            "Capability host operation failed.",
          );
    send({
      type: "host.error",
      ...(candidate.type === "host.capabilities.set"
        ? { requestId: candidate.requestId }
        : {}),
      code: safe.code,
