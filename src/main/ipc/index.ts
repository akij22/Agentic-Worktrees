...
export const configureMarketplaceIpc = (service: CapabilityDistributionService | null): void => {
	...
	marketplaceEvents.configure(service, (event) => {
		if (!service) return;
		const publicEvent = createMarketplaceHandlers(service, requireSkillService()).event(event);
...
const authStatusResponse = async (operation: () => Promise<unknown>) =>
...
const handleCreateLocalBranch = async (
	...
	if (!isLocalRepository(repo)) {
		throw new Error(
			"Creating branches is only supported for local repositories.",
		);
	}
	if (!repo.localRootPath) {
		throw new Error(`Local repository path not found: ${repo.id}`);
...
const marketplaceHandlers = () => createMarketplaceHandlers(
	marketplaceDistribution ?? (() => { throw new Error("Marketplace service is unavailable."); })(),
	requireSkillService(),
);
...
const handleIntelligenceResolutionList = (
	...
	return conflictResolutionSessionSchema
		.array()
		.parse(conflictIntelligenceService.listSessions(request));
};
...
const handleIntelligenceIntegrationOpen = async (
	...
	if (!session.retained || !session.integrationPath) {
		throw new Error(
			`Conflict session has no retained Integration Worktree: ${request.sessionId}`,
		);
	}
...
export const registerIpcHandlers = (): void => {
	...
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_LIST, (_event, raw) => marketplaceHandlers().list(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_INSPECT, (_event, raw) => marketplaceHandlers().inspect(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_INSTALL, (_event, raw) => marketplaceHandlers().install(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_CHECK_UPDATES, (_event, raw) => marketplaceHandlers().checkUpdates(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_UPDATE, (_event, raw) => marketplaceHandlers().update(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_INSPECT_REMOVAL, (_event, raw) => marketplaceHandlers().inspectRemoval(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_REMOVE, (_event, raw) => marketplaceHandlers().remove(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_CANCEL, (_event, raw) => marketplaceHandlers().cancel(raw));
	ipcMain.handle(IPC_CHANNELS.MARKETPLACE_RETRY_PENDING_MIGRATIONS, (_event, raw) => marketplaceHandlers().retryPendingMigrations(raw));
	...
	ipcMain.handle(IPC_CHANNELS.GITHUB_AUTH_START, () =>
		githubAuthService
			.startLogin()
			.then((value) => githubDeviceChallengeSchema.parse(value)),
	);
	...
	ipcMain.handle(
		...
		requireAuthenticated(handleGithubListRepos),
	);
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		requireAuthenticated(handleGithubListBranches),
	);
	ipcMain.handle(
		IPC_CHANNELS.GIT_CREATE_BRANCH,
		requireAuthenticated(handleCreateLocalBranch),
	);
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		requireAuthenticated(handleWorktreeCreate),
	);
	ipcMain.handle(
		IPC_CHANNELS.WORKTREE_LIST,
		requireAuthenticated(handleWorktreeList),
	);
	ipcMain.handle(
		IPC_CHANNELS.WORKTREE_LIST_ALL,
		requireAuthenticated(() => listAllWorktrees()),
	);
	ipcMain.handle(
		IPC_CHANNELS.EDITOR_LIST_AVAILABLE,
		requireAuthenticated(() => listAvailableEditors()),
	);
	ipcMain.handle(
		IPC_CHANNELS.EDITOR_OPEN,
		requireAuthenticated(handleEditorOpen),
	);
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		requireAuthenticated(() => getAgentInstallationStatus()),
	);
	ipcMain.handle(
	...
	ipcMain.handle(
		...
		requireAuthenticated(() => listAgentWorktrees()),
	);
	ipcMain.handle(
